...
public class Client implements Callable<NetworkClient>
{
    ...
    public NetworkClient call() throws Exception {
        ...
        for (;;) {
            ...
            if(playerNumber == 1)
                networkClient.sendMove(getBestMove(playerNumber, 4)); // 6 oder 7 geht auch aber stürzt noch bei vielen möglichen Zügen ab
            else
    ...
    public int GetMoveMinMax(int[][] pitch, int player, int depth, int maxDepth, int alpha, int beta) {
        ...
        if (player == setup.playerNumber)
        {
            ...
            for (Move move : getValidMoves(pitch, tops)) {
                ...
                if (depth + 1 < maxDepth) {
                    int nextMovePoints = GetMoveMinMax(pitch, setup.getNextPlayer(player), depth + 1, maxDepth, currentAlpha, beta);
...
//            }
            if (depth + 1 < maxDepth) {
                int nextMovePoints = GetMoveMinMax(pitch, setup.getNextPlayer(player), depth + 1, maxDepth, alpha, currentBeta);
